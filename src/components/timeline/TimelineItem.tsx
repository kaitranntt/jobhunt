...
export default function TimelineItem({ activity }: TimelineItemProps) {
  ...
  return (
    <li
      className="relative pb-8 last:pb-0 glass-ultra rounded-glass-sm p-4 shadow-glass-subtle glass-interactive animate-spring-bounce-in"
    ...
    >
      <div className="flex gap-4">
        ...
        <div className="flex-1 space-y-1">
          <div className="flex items-start justify-between gap-2">
            <h3 className="font-semibold text-base text-label-primary leading-tight">
              {activity.title}
            </h3>
            <time
          ...
          {activity.application_name && (
            <p className="text-xs text-label-tertiary font-medium">{activity.application_name}</p>
          )}
        </div>
      </div>
      ...
      {/* Connecting line */}
      <div
        className="absolute left-9 top-14 -ml-px h-full w-0.5"
