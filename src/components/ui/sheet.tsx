  ...
  "fixed z-50 gap-4 p-6 shadow-lg transition ease-in-out data-[state=closed]:duration-300 data-[state=open]:duration-500 data-[state=open]:animate-in data-[state=closed]:animate-out",
  {
    variants: {
      ...
      variant: {
        ...
        glass:
          "bg-[var(--glass-light)] backdrop-blur-[20px] [-webkit-backdrop-filter:blur(20px)] saturate-[180%] border-[var(--glass-border-strong)]",
      },
    },
    defaultVariants: {
      side: "right",
...
>(({ side = "right", variant = "default", className, children, ...props }, ref) => (
  <SheetPortal>
    <SheetOverlay variant={variant || "default"} />
    <SheetPrimitive.Content
      ref={ref}
      className={cn(
        sheetVariants({ side, variant }),
        variant === "glass" && "[transition-timing-function:cubic-bezier(0.34,1.56,0.64,1)]",
        className
      )}
      {...props}
    >
      <SheetPrimitive.Close
        className={cn(
          ...
          variant === "default" && "data-[state=open]:bg-secondary",
          variant === "glass" &&
